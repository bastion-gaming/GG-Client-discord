...
async def on_ready():
    ...
    GGconnect = ge.ZMQ()
    print('------\n')
    activity = discord.Activity(type=discord.ActivityType.playing, name="{0}help".format(PREFIX))
...
async def on_message(message):
    if not (message.author.bot) and message.content.startswith(PREFIX):
        if GGconnect:
            if not message.content.startswith("{}help".format(PREFIX)):
...
client.load_extension('gems.gemsMarket')
...
client.load_extension('gems.gemsCasino')
